...
public class OcrProviderDataHandler(InvocationContext invocationContext)
    : BaseInvocable(invocationContext), IAsyncDataSourceItemHandler
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var request = new RestRequest("/ai/image/ocr", Method.Get);

        var providers = await client.ExecuteWithErrorHandling<List<OcrProviderDto>>(request);
